...
- After completing any review or challenge (after presenting findings and any required fixes), update the active plan file's Completion Checklist. The active plan is the most recent file under `$(git rev-parse --path-format=absolute --git-common-dir | sed 's|/.git$||')/docs/plans/*.md` whose frontmatter does not include `status: complete`. Edit the plan: change `- [ ] Second-opinion review` to `- [x] Second-opinion review`. If you ran an adversarial challenge, add a note: `- [x] Second-opinion review (challenge: <one-line summary>)`. The coding-team lifecycle hook reads this checkbox to enforce the gate; without the edit the pipeline will block at completion. If no active plan file is found (standalone `/second-opinion` outside a coding-team session), skip this step.
