...
## 1. Real Over Mocks
Use real implementations in tests — see `~/.claude/reference/test-files.md` for the base rule. Only mock when a dependency is physically impossible to run locally: external paid API with no test mode (e.g. Stripe, Bittensor), hardware not in CI (e.g. GPU), or a local LLM / Ollama model not installed locally.
...
## 17. Right-Sized Code
Code should be "engineered enough" — both under-engineering (fragile, missing guards, hacky) and over-engineering (premature abstraction, unrequested features, speculative complexity) are failures. Decide the right size BEFORE writing, not at review: build only what the spec/AC requires → use the simplest construct that satisfies it → abstract at the second real call site, not the first → if scope balloons (many files, new services), challenge it before coding. NEVER delete a defensive guard (validation, error handling, retry, null/boundary check) to "simplify" — right-sized never means fewer guards. Known rationalization: "YAGNI says skip the validation" — YAGNI governs unbuilt features, never live safety code.
- **Enforcement**: the `ct-simplify-auditor` PROTECTED set (`agents/ct-simplify-auditor.md:50-119`) is the inline correctness fence — a defensive construct is challengeable only by naming the upstream guarantee that makes it redundant (the `consider` tier), never a bare "remove"; graduated learning C1 (`_graduated/`) is the canonical single-gate-path-trust case.
