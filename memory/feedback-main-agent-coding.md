...
**How to apply:** Phase 5 must have an explicit prohibition: the main agent orchestrates (spawn teammates, read results, decide next steps) but never uses Edit, Write, or runs tests directly. Only teammate spawns (or Agent tool if teams unavailable) for code changes during execution.
