...
8. Codesight-mcp is preferred for code analysis when available. Reindex stale indexes. Grep/Read are fallbacks when MCP is unavailable or calls fail. It fails in TWO shapes and both have been observed on this repo: (a) an explicit error — a repo path outside the trusted prefix is rejected; (b) worse, a silent EMPTY result with a nonzero files_searched, which reads as "no callers found" and gets wrongly trusted. Treat zero results on a symbol you have reason to believe exists as a TOOL failure, not a code fact: confirm with Grep before concluding anything. Do NOT skip the underlying check because the tool came back clean.
...
11. Never retry a failed MCP tool more than once. Mark it unavailable for the rest of the session and degrade to built-in tools (Glob, Grep, Read). This applies to every agent and to the orchestrator. Named rationalizations, all false: "Maybe it's back up now" — it isn't; "I need this specific MCP tool" — the built-ins do the same job less efficiently; "Let me try a different query" — the server is down, not the query. Degrading is not skipping: perform the underlying check with Grep/Read.
...
16. Delegation is the default, not a choice. Do NOT offer self-execution as an option ("should I just execute directly?"). Spec clarity is not a reason to self-execute — all code changes delegate regardless. Named rationalization: "the spec is already clear."
...
18. Report every defect you find, regardless of when it was introduced. "Pre-existing" and "not a regression" are NOT valid reasons to skip a finding — a bug is a bug. If you cannot complete a review (files inaccessible, empty file list, missing spec/plan), report **Status: BLOCKED — [reason]**. Do NOT guess, fabricate findings, or return an empty report; BLOCKED is always better than an unreliable review.
