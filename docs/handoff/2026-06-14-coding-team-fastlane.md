...
- **Phase 6 verification:** instruction-only diff (vs `fe2bfd4`) → code suite (554 passed) unaffected; no doc-drift in README/CLAUDE.md; deploy clean. Plan left `status: planned` (never in-progress). Unrelated WIP (`skills/second-opinion/codex-learnings.md`, `memory/patterns.md`) kept isolated/untouched.
- **Post-exec Codex `/second-opinion review` (2026-06-14): FAIL → remediated `bf301da`.** Codex caught a temporal bug the opus audit missed: the per-task verification GATE (`execution.md` step 9, in-loop) referenced the EFFECTIVE tier that is only recomputed later (`:157`, after the loop). Fix: per-task verification gates on the PLANNED tier (growth covered at exit by the unconditional full-suite + effective-tier QA reviewer); removed "per-task verification sweep" from the recompute's effective-tier carry-list. Also separated doc-drift's tier gate from its orthogonal no-impact early-exits (`doc-drift-scan.md`). Codex findings 3 & 4 (post-exec/Phase-6 local recompute) = by-design standalone fallbacks (identical classifier, can't diverge), not changed. Learning-capture candidate noted (forward-reference-to-uncomputed-value pattern), NOT written (codex-learnings.md is user WIP).

**Landing: stacked PR.** Branch `coding-team-weight-ladder-batch2` → base `coding-team-weight-asymmetry-fastlane` (PR #73 head), so the PR diff is only the instruction files. After PR #73 merges to main, rebase Batch 2 onto main.
