...
Read `~/.claude/reference/finding-integrity.md` before starting. Summary: report behavioral defects regardless of when introduced ("pre-existing" is not a valid reason to skip a finding), and if you cannot complete the review, report **Status: BLOCKED — [reason]** rather than guessing or fabricating findings.
