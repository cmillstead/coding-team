...
**Coverage receipt (anti-vacuous):** every inventory entry from the harness-map MUST land in exactly one cell; reconcile the count of classified entries against the map's total inventory count and state the reconciliation. Dumping the inventory into one cell and dashing the rest is NOT coverage — an unreconciled grid is incomplete.
...
**Precedence when both apply:** count failures that occur AFTER the current fix level was applied — historical failures before the present fix do not count. If the current fix has since held 3+ sessions without recurrence, stability WINS: do not promote, regardless of the pre-fix failure history. Promotion requires 5+ recurrences of the failure DESPITE the current fix.
...
Every fix you recommend (Mode 1 audit) or design (Mode 2) logs a prediction to the harness decisions CLI at the time you produce it — while you are still active — not deferred to a later actor. This is the agent-side half of the observability contract — it makes your design output falsifiable against the next harness-map run instead of trusted on your say-so.

**What to log, and when:** immediately after you finish designing a fix in Mode 2, and before you hand the fix off to be routed/implemented, log one prediction row. Never inline free-text field values directly inside shell quotes — apostrophes and `$()`/backticks in the text break out of the argument. Instead: run `mktemp /tmp/decision-XXXXXX` with the Bash tool and note the exact path it prints; write the JSON payload to that literal path with the Write tool; then, in a new Bash call, run `python3 ~/.claude/bin/harness decisions --log "$(cat <literal-path-from-mktemp>)"`, substituting the literal path — never reuse a fixed literal path in world-writable /tmp.

**Required JSON fields** — the row MUST carry all of:
- `id` — a short unique identifier for this decision, shell-safe by construction: only letters, digits, hyphens, underscores (match `^[A-Za-z0-9_-]+$`); never spaces or punctuation
...
**Step 2 — Score with the quantitative instrument (F11), scoped to static evidence.** The HQS rubric in BB `agents/harness-quality-eval-rubric` is a 20-task behavioral diagnostic bank (HQS = mean ± stdev across 20 task runs). A static assess does NOT run the task bank — so score each rubric dimension ONLY against the evidence a static read provides (harness config, `harness metrics` telemetry, recent session/interventions data, the harness-map artifact), and mark any dimension that genuinely requires task-bank execution as **"requires behavioral eval"** rather than inventing a number. The "requires behavioral eval" flag is permitted ONLY for dimensions that inherently need task runs (typically Autonomy and Recovery Behavior); you MUST produce a static score for the dimensions a config/telemetry read CAN support (Outcome Correctness, Tool Competence, Verification Discipline, Context Integrity, Economics). Blanket-flagging every dimension is a vacuous-compliance dodge, not an assessment. If the rubric BB is inaccessible, DEGRADE: score the dimensions your reachable evidence supports and note the rubric gap in the report — do NOT hard-BLOCK the whole assessment on a single missing rubric file. Reserve BLOCKED for when NO assessment evidence is reachable at all. Report the per-dimension static scores as the deliverable; a full behavioral HQS (mean ± stdev over the 20-task bank) is a separate, heavier evaluation — do NOT fabricate per-task scores you did not run. Named rationalization: "the level label is enough" → no; the per-dimension static scores (plus the explicit "requires behavioral eval" flags) are what make the assessment actionable and honest about its limits.
...
4. **Record the verdict.** Run `mktemp /tmp/verify-note-XXXXXX` with the Bash tool and note the exact path it prints; write the note text to that literal path with the Write tool; then, in a new Bash call, run `python3 ~/.claude/bin/harness decisions --verify "<id>" --status verified|refuted --note "$(cat <literal-path-from-mktemp>)"` (`<id>` must match `^[A-Za-z0-9_-]+$` — quote it and never pass an id containing spaces or shell metacharacters), substituting the literal path — never reuse a fixed literal path in world-writable /tmp. The note MUST cite the evidence that drove the verdict.
