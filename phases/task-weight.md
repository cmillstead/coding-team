...
- **Payment / billing / financial effect:** alters payment capture, billing, charges, refunds, invoicing, pricing, or any money-moving / financial-record path — in ANY file, regardless of name. (Harness already treats `payment` as security-sensitive: `planning.md:27`, `planning-next-steps.md:11`, `execution.md`'s step 8.5 security-escalation grep, CLAUDE.md high-stakes.)
...
- **Behavioral-instruction-file edit:** ANY edit to `phases/*.md`, `SKILL.md`, `agents/*.md`, `CLAUDE.md`, `hooks/*`. (Same high-impact surface `hooks/write-guard.py`'s `BEHAVIORAL_INSTRUCTION_DIRS` gates — allowed only when the active plan declares the path under `instruction_files:` — and `phases/post-execution-review.md`'s step 1e/1f (CC instruction-file check) always flags. A 1-line instruction-file change can cascade across every pipeline run.)
