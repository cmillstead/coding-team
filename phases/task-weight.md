...
- **Behavioral-instruction-file edit:** ANY edit to `phases/*.md`, `SKILL.md`, `agents/*.md`, `CLAUDE.md`, `hooks/*`. (Same high-impact surface `hooks/write-guard.py`'s `BEHAVIORAL_INSTRUCTION_DIRS` always-delegates and `phases/post-execution-review.md`'s step 1e/1f (CC instruction-file check) always flags. A 1-line instruction-file change can cascade across every pipeline run.)
