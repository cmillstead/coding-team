...
**"Run it from a directory where the gate is dormant"** — Choosing a working directory so `find_active_plan()` returns `None` disarms the guard; it is a bypass, not a workaround (`rules/hook-bypass.md`). The correct remedy for a legitimately-blocked instruction-file edit is letting the plan reach `status: complete`, repairing stale frontmatter, or the documented `WRITE_GUARD_ALLOW_INSTRUCTION_EDIT=1` override — never relocating cwd.
