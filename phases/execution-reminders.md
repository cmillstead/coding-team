...
> **Pre-execution check:** Did `/second-opinion review` RUN on the plan? If Codex is available and the EFFECTIVE (or planned) tier is ≥ Medium and it did not run, run it now against the plan file, then continue to the first task. The plan second-opinion gate is required at tier ≥ Medium per `phases/task-weight.md`; the user override (see `planning-next-steps.md`) applies.
>
