...
Dispatch a **Planning Worker** via Agent tool. Pass: design doc + full project context.
...
Read `phases/planning-reference.md` and use its Code Intelligence tool table (the `mcp__codesight__query` operations plus the `gh search` and `mcp__context-keep__search_memories` queries) to understand the codebase before decomposing tasks. Do NOT skip codebase analysis — use whichever tools are available.
