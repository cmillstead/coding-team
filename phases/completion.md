...
| Working tree clean | `git status --porcelain --untracked-files=all` shows nothing except the active plan file (`--untracked-files=all` REQUIRED — default collapses an untracked dir to `?? docs/` and can hide the plan) | Commit or discard uncommitted work before marking the plan complete |
...
Read `phases/completion-reference.md` and follow its Decision Log instructions — the tier gate, the decision-record template, the ContextKeep persistence step, and the memory-nudge hand-off.
