...
> Loaded by the orchestrator from `cookbook/phases/execution.md` after all Phase 5 tasks are complete. After following these instructions, proceed to Phase 6 (read `cookbook/phases/completion.md`).
...
1. Evaluate risk signals against the completed diff. Run these commands:
   ...
   e. Check for CC instruction files: `git diff $(git merge-base HEAD main) --name-only | grep -iE "cookbook/phases/|agents/|skills/|prompts/|CLAUDE.md|SKILL.md"`
