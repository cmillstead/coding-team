...
**Pre-flight: Session state (FIRST ACTION).** Write the session state file BEFORE any reads, edits, or agent dispatches. This is the first action in Phase 5 — no exceptions.

```bash
python3 -c "import json, time; json.dump({'phase': 'execution', 'ts': time.time()}, open('/tmp/coding-team-session.json', 'w'))"
```

Known rationalization: "I'll set up the session after this quick edit" — the session marker must be written BEFORE any Phase 5 edits. Without it, write-guard cannot detect orchestrator bypass.
...
For each task in plan:
  ...
  IMPLEMENTER (see ~/.claude/agents/ct-builder.md)
  ...
  AUDIT PASS
  Read `cookbook/phases/audit-loop.md` and follow its instructions for the audit pass.
...
**MANDATORY:** Read `cookbook/phases/execution-reminders.md` and print the **Post-Execution Checklist** section. This fires regardless of the 3-task reminder cadence.
...
2. Dispatch the QA reviewer via Agent tool (model: sonnet, subagent_type: Explore — read-only):
   ...
   - See `~/.claude/agents/ct-qa.md` for the full prompt template
...
Read `cookbook/phases/doc-drift-scan.md` and follow its instructions.
...
At every phase transition and before any completion claim, follow the `/verify` skill (`commands/verify.md`). No "should pass," no "looks correct," no trusting agent reports without independent verification.
...
After every 3 completed tasks, read `cookbook/phases/execution-reminders.md` and print the progress template. Also print the orchestrator self-check.

## Debugging Detour Reminders

When entering or exiting the debug protocol, read `cookbook/phases/execution-reminders.md` and print the appropriate template.

## Next Steps

After all tasks are executed and verified, read `cookbook/phases/post-execution-review.md` and follow its instructions. It handles risk signal evaluation, optional Codex review, and the completion handoff to Phase 6.
