...
2. Dispatch audit agents IN PARALLEL via Agent tool (spec reviewer and simplify auditor as read-only Explore; harden auditor and harness engineer as general-purpose to allow Bash tool access):
   ...
   b. Simplify auditor (see ~/.claude/agents/ct-reviewer.md) — "is there a simpler way?"
   c. Harden auditor (see ~/.claude/agents/ct-harden-reviewer.md) — "what would an attacker try?"
   d. Prompt-craft auditor (see ~/.claude/agents/ct-prompt-reviewer.md) — triggers when BOTH:
      (i) Task has PROMPT_CRAFT_ADVISORY annotation, AND
      (ii) Modified files include at least 1 file matching: `cookbook/phases/*.md`, `agents/*.md`, `prompts/*.md`, `skills/*/SKILL.md`, `SKILL.md`, `CLAUDE.md`, `memory/*.md`
