...
**Design default:** When writing a CI step that calls any venv-dependent tool, add `python -m venv .venv && . .venv/bin/activate` at the top of that same `run:` block — `actions/setup-python` provides an interpreter, not a venv.
