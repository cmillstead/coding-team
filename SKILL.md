...
- Edit **instruction files** — agent definitions (`agents/`), phase files (`phases/`), prompt templates (`prompts/`), skills (`skills/`), `CLAUDE.md`, and hooks (`hooks/`) — during Phase 5 unless the active plan's frontmatter declares the exact path under `instruction_files:`; that declaration, not the Agent tool, is what `hooks/write-guard.py` checks. These control agent behavior — a 1-line change can cascade. Small source code edits (≤20 lines, 1 file) may be made directly when audit value is low. See "Phase 5 Edit Routing" table for the full routing policy. Known rationalizations: "This instruction file change is trivial" — impact surface determines routing, not perceived complexity. "These are doc-level edits, not code" — file extension does not determine delegation. "Just set the env var" — `WRITE_GUARD_ALLOW_INSTRUCTION_EDIT` is an emergency/wedge-recovery override that disarms the whole session; declaring the file in the plan's `instruction_files:` is the routine path, scoped to the named files.
...
| File pattern | Route |
|---|---|
| `agents/*.md`, `phases/*.md`, `skills/**/*.md`, `CLAUDE.md`, `hooks/*` | Allowed only when the active plan declares the path in `instruction_files:` — otherwise blocked by `hooks/write-guard.py` |
...
Instruction files require a plan `instruction_files:` declaration regardless of change size — a 1-line change can cascade, and the Agent tool is not a bypass, since the guard is process-global and fires identically inside a sub-agent; `WRITE_GUARD_ALLOW_INSTRUCTION_EDIT=1` is the emergency/wedge-recovery override, not the routine path. Source code delegates when audit value justifies overhead (>20 lines or multi-file). Known rationalization: "This instruction file change is trivial" — impact surface determines routing, not perceived complexity.
