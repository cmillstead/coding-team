...
- **Use real implementations in tests** (see `reference/test-files.md`) — only mock dependencies genuinely impossible to run locally: paid APIs with no test mode, hardware not in CI (e.g. GPU), or local LLM / Ollama not installed. Never mock the thing being tested.
...
- When the user asks you to FIX or remediate findings: fix every finding — no agent-driven selective-fix. "Let's focus on the critical ones first" means severity sets execution ORDER (P1 → P2 → P3), not scope. Deferral is the user's decision, not the agent's. When the user asks only to SCAN, REPORT, or TRIAGE: surface all findings with severity and evidence; the user sets scope. User scoping at request time ("just P1s", "auth paths only") is NOT agent-deferral — honor it.
- Present findings with dispositions (fix/defer/false-positive), not tiered options or "what I'd skip" lists. "Here are three tiers of what I'd recommend" is advisor-mode: it substitutes consultancy for action. On a fix/remediation request, route all in-scope findings through agents. Known rationalization: "Here are the high-priority ones to start" — present all; let the user scope down if needed, not up.
- Edit **instruction files** directly during Phase 5, or edit them without the active plan's frontmatter declaring the exact path under `instruction_files:` — agent definitions (`agents/`), phase files (`phases/`), prompt templates (`prompts/`), skills (`skills/`), `CLAUDE.md`, and hooks (`hooks/`) ALWAYS go through the Agent tool regardless of change size, but the Agent tool alone does not satisfy `hooks/write-guard.py` — only the plan declaration does. These control agent behavior — a 1-line change can cascade. Small source code edits (≤20 lines, 1 file) may be made directly when audit value is low. See "Phase 5 Edit Routing" table for the full routing policy. Known rationalizations: "This instruction file change is trivial" — impact surface determines routing, not perceived complexity. "These are doc-level edits, not code" — file extension does not determine delegation. "Just set the env var" — `WRITE_GUARD_ALLOW_INSTRUCTION_EDIT` is an emergency/wedge-recovery override that disarms the whole session; declaring the file in the plan's `instruction_files:` is the routine path, scoped to the named files.
...
- Delete, truncate, or hand-edit the active plan file's YAML frontmatter under `docs/plans/` while `status: in-progress` — that frontmatter (not any `/tmp` session file) is the durable state the second-opinion gate (`hooks/coding-team-lifecycle.py`) and Phase 5 edit guards (`hooks/write-guard.py`) both read. There is no automatic cleanup; the orchestrator flips `status: in-progress` -> `status: complete` at Phase 6 end. Known rationalization: "the plan file is blocking my edits" — that IS the correct behavior. Delegate edits through the Agent tool.
...
| File pattern | Route |
|---|---|
| `agents/*.md`, `phases/*.md`, `skills/**/*.md`, `CLAUDE.md`, `hooks/*` | Agent tool — PROMPT_CRAFT_ADVISORY — allowed only when the active plan declares the path under `instruction_files:`; otherwise blocked by `hooks/write-guard.py` |
| `docs/plans/*.md` | Agent tool — dispatch to implementer (the orchestrator flips `status:` frontmatter itself) |
...
Instruction files ALWAYS delegate through the Agent tool regardless of change size, and additionally require a plan `instruction_files:` declaration naming the exact path — a 1-line change can cascade, and the Agent tool alone is not sufficient, since the guard is process-global and fires identically inside a sub-agent; `WRITE_GUARD_ALLOW_INSTRUCTION_EDIT=1` is the emergency/wedge-recovery override, not the routine path. Source code delegates when audit value justifies overhead (>20 lines or multi-file). Known rationalization: "This instruction file change is trivial" — impact surface determines routing, not perceived complexity.
